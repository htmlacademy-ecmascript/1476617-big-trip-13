...
class EventsList {
  ...
  setItemEditedHandler(index) {
    ...
    const newElement = new _trip_event_form_form__WEBPACK_IMPORTED_MODULE_1__["default"]({tripEvent: this._tripEvents[index], unsetItemEditedHandler: () => this.unsetItemEditedHandler(index)}).getElement();
    this._element.replaceChild(newElement, oldElement);
  }

  unsetItemEditedHandler(index) {
    const oldElement = this._element.children[index];
    const newElement = new _trip_event_item_event_item__WEBPACK_IMPORTED_MODULE_2__["default"]({tripEvent: this._tripEvents[index], setItemEditedHandler: () => this.setItemEditedHandler(index)}).getElement();
    this._element.replaceChild(newElement, oldElement);
  }
  ...
  getElement() {
    if (!this._element) {
      ...
      const tripEventItems = tripEvents.map((tripEvent, index) => {
        return new _trip_event_item_event_item__WEBPACK_IMPORTED_MODULE_2__["default"]({tripEvent, setItemEditedHandler: () => this.setItemEditedHandler(index)}).getElement();
      });
